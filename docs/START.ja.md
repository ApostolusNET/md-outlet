...
**言語:** [English](START.md) · [日本語](START.ja.md)

このツールは **簡易エディター＋PDF 出力** です。  
ブラウザでプレビューしながら書き、同じ見た目の PDF を保存できます。

**信頼:** 開くのは信頼できる文書だけにしてください。出所不明の Markdown には、プレビュー／PDF に影響する HTML が含まれることがあります。詳しくは [README（セキュリティ／信頼境界）](../README.ja.md#セキュリティ信頼境界) を参照。

**覚えておくとよい既定:**

- プロファイルの `allowHtml`: `true` / `"breaks"` は改ページ用ヘルパーのみ。任意 HTML が必要なときだけ `"raw"`（信頼できる文書限定）。プレビューの CSP は既定でスクリプト禁止、`"raw"` のときだけ緩和。
- UI 起動ごとにセッション API トークン（`X-MD-Outlet-Token`）を発行。自動化では `MD_OUTLET_API_TOKEN` で上書き可。
- **パッケージ外かつ開いている文書フォルダの外**への保存／PDF 出力は、UI で明示確認が必要です。
...
| 「タブが上限（3）」 | 文書タブは最大 3。**同じファイルの再送／再オープン**は既存タブにフォーカス（成功）。**新しいファイル**（MD 以外含む）は拒否し、送る側ダイアログ＋ UI トーストで知らせる。不要なタブを × で閉じてから再度開く／送る |
| 「送る」で XML/CSV などだけ失敗する | タブ満杯時は仕様どおりエラー（アクティブタブの中身は差し替えない）。先にタブを閉じてから送る |
| 「送る」で Exit code / 開けない | API トークン追加後は `start-ui-sendto.ps1` がトークン付きで接続します。**いまのフォルダのスクリプトが使われているか**確認（古いコピーを「送る」していると失敗）。必要なら `install-sendto.bat` を再実行。すでに UI があるときはブラウザタブを一度閉じてから再送る |
| 「送る」で何も起きない（エラーもブラウザも出ない） | 裏で UI サーバだけ残っていることが多い。もう一度「送る」するとブラウザが開くようにした。だめならタスクマネージャで `node`（ポート 5760）を終了してから `start-ui.bat` か再度「送る」 |
